...
* `ST_AddMeasure(geometry A, double from, double to) <http://postgis.net/docs/ST_AddMeasure.html>`_: Converts a linestring or multilinestring into a **measured path** (M values). Think of it as a **road** where you want to assign **house numbers** progressively from start to end. This function assigns "measure" values (like house numbers, kilometers, or travel times) that increase linearly along the line. If the geometry has **no M coordinates**, they are added. If it **already has M values**, they are overwritten. Only LINESTRING and MULTILINESTRING are supported.

  **Example (house numbers):** START (house 10) --- intermediate points --- END (house 100) → each point gets a proportional value.